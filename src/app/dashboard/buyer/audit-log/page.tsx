...
export default function BuyerAuditLogPage() {
  ...
  const handleExportCSV = () => {
    const headers = ["1", "Transaction ID", "Action", "User", "Role", "Time", "Status"];
    ...
    link.href = url;
    link.download = `buyer_audit_log_${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
  ...
  const handleMonthlyReport = () => {
    ...
    const link = document.createElement("a");
    link.href = url;
    link.download = `buyer_monthly_report_${new Date().toISOString().split('T')[0]}.txt`;
    link.click();
    URL.revokeObjectURL(url);
  };
  ...
  return (
    <DashboardShell
      ...
      breadcrumbs={[{ href: "/dashboard/buyer", label: "Buyer Home" }, { label: "Audit Log" }]}
    >
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
        <p className="text-slate-500 dark:text-slate-400 max-w-2xl">
          Comprehensive history of all activities within the escrow lifecycle. Essential for regulatory compliance and dispute resolution.
        </p>

        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-end gap-3">
          <Button onClick={handleExportCSV} variant="outline" className="border-slate-300 text-slate-700 hover:bg-slate-50 flex items-center gap-2">
            <Download className="w-4 h-4" /> Export CSV
          </Button>
          <Button onClick={handleMonthlyReport} className="bg-primary hover:bg-primary/90 text-white font-bold flex items-center gap-2">
                ...
                {auditData.length === 0 ? (
                  ...
                  auditData.map((row) => (
                    <tr key={row.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                      <td className="px-6 py-4 font-mono font-bold text-primary truncate max-w-[120px]">
                        {row.id}
                      </td>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl p-6 w-full max-w-md relative">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><p className="text-sm font-semibold text-slate-500">Counterparty</p><p className="mt-1 text-slate-900 dark:text-white">{selectedRow.sellerCompany}</p></div>
                <div><p className="text-sm font-semibold text-slate-500">Role</p><p className="mt-1 text-slate-900 dark:text-white">Seller</p></div>
              </div>
            ...
            <div className="mt-8 flex justify-end">
              <Button onClick={() => setSelectedRow(null)} className="bg-slate-900 hover:bg-slate-800 text-white dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-slate-200">
                Close
              </Button>
            </div>
          </motion.div>
