...
export default function SellerContactsPage() {
  ...
  return (
    <DashboardShell
      ...
      breadcrumbs={[{ href: "/dashboard/seller", label: "Seller Home" }, { label: "Contacts" }]}
    >
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
        ...
        <div className="border-b border-slate-200 dark:border-slate-800">
          <div className="flex gap-8">
            <button
              ...
              className={`flex items-center gap-2 border-b-2 pb-4 px-2 font-bold transition-all ${tab === "received" ? "border-primary text-primary" : "border-transparent text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200"}`}
            >
              Received Requests
            ...
            <button
              type="button"
              onClick={() => setTab("sent")}
              className={`flex items-center gap-2 border-b-2 pb-4 px-2 font-bold transition-all ${tab === "sent" ? "border-primary text-primary" : "border-transparent text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200"}`}
            >
              Sent Requests
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="flex gap-2">
            <Button variant="outline" className="border-slate-300 text-slate-700 hover:bg-slate-50 flex items-center gap-2"><ArrowDownAZ className="w-4 h-4" /> Sort</Button>
            <Button variant="outline" className="border-slate-300 text-slate-700 hover:bg-slate-50 flex items-center gap-2"><Download className="w-4 h-4" /> Export</Button>
          </div>
        </div>
          ...
          {list.map((card, i) => (
            ...
            >
              <div className="flex justify-between items-start mb-4">
                ...
                <span className="px-3 py-1 rounded-xl text-[11px] font-bold uppercase tracking-wider bg-amber-500/10 text-amber-500 border border-amber-500/20">{card.status}</span>
              </div>
              ...
              <div className="flex gap-3 mb-4">
                ...
                <Button variant="outline" className="flex-1 border-slate-300 text-slate-600 hover:text-red-600 hover:border-red-300 hover:bg-red-50">Reject</Button>
              </div>
              <p className="text-[11px] text-slate-500 flex items-center gap-1"><Calendar className="w-3.5 h-3.5" /> Requested on {card.date}</p>
            </motion.div>
          ))}
          {list.length === 0 && (
            <p className="text-slate-500 dark:text-slate-400 col-span-full text-sm">
              No contacts yet. New requests will appear here as you connect with partners.
            </p>
          )}
        </div>
      </motion.div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 shadow-2xl">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">
              Send a connection request to a buyer or supplier you want to work with.
            </p>
