...
export function DashboardShell({
  ...
  return (
    <div className="min-h-screen flex text-foreground font-display bg-background">
      ...
      <aside className="hidden lg:flex fixed left-0 top-0 z-20 w-64 h-full border-r border-border flex-col bg-card">
        {sidebarContent}
      </aside>
        ...
        {isSidebarOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed left-0 top-0 z-40 w-72 max-w-[85vw] h-full border-r border-border flex flex-col bg-card shadow-2xl lg:hidden"
            >
              {sidebarContent}
            </motion.aside>
          </>
      ...
      {/* Main – ml-64 only on desktop */}
      <main className="relative z-10 ml-0 lg:ml-64 flex-1 flex flex-col min-h-screen w-full">
        {/* Top Header */}
        <header className="h-16 md:h-20 border-b border-border bg-card/80 backdrop-blur-md flex items-center justify-between px-4 md:px-6 lg:px-8 sticky top-0 z-10">
          <div className="flex items-center gap-3 min-w-0">
            {/* Hamburger – visible only below lg */}
            <button
              type="button"
              onClick={() => setIsSidebarOpen(true)}
              className="lg:hidden p-2 -ml-1 rounded-lg text-muted-foreground hover:bg-muted transition-colors shrink-0"
          ...
          <div className="flex items-center gap-3 md:gap-6">
            ...
            <div className="relative hidden sm:block">
              ...
              <input
                ...
                className="pl-10 pr-4 py-2 rounded-lg bg-muted border border-border focus:ring-2 focus:ring-primary text-sm w-40 md:w-56 lg:w-64 text-foreground placeholder:text-muted-foreground"
              />
            </div>
              ...
              <div className="pl-2 md:pl-4 border-l border-border">
                <DropdownMenu open={isProfileOpen} onOpenChange={setIsProfileOpen}>
                  <DropdownMenuTrigger asChild>
                    <div className="flex items-center gap-3 cursor-pointer select-none rounded-lg p-1 pr-1 md:pr-3 hover:bg-muted transition-colors">
                      ...
                      <div className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-primary/20 flex items-center justify-center text-primary font-bold overflow-hidden">
                  ...
                  >
                    <motion.div
                      ...
                      transition={{ duration: 0.15, ease: "easeOut" }}
                    >
                      <div className="px-2 py-1.5 pb-3 border-b border-border mb-2 md:hidden">
        ...
        {/* Page content */}
        <div className="p-4 md:p-6 lg:p-8 space-y-6 md:space-y-8 flex-1">{children}</div>

        {/* Footer */}
        <footer className="p-4 md:p-6 lg:p-8 mt-auto border-t border-border text-muted-foreground text-xs flex flex-col sm:flex-row justify-between gap-3">
